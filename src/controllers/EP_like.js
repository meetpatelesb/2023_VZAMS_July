...
var fetch_like = async(req, res) => {
    ...
    var tweet_tbl = `SELECT * from twitter.tweet_master WHERE tweet_id = '${tweet_id}'`
    ...
    var like_select = `SELECT * FROM twitter.like_master WHERE like_user_id='${user_id}' AND like_tweet_id='${tweet_id}'`
    var select_result = await queryExecute(like_select)

    //dislike update ------------------------------------------------------------------
    if (select_result[0]) {
        if (select_result[0].activate == 1) {
            var like_delete = `UPDATE twitter.like_master SET activate='0' WHERE like_tweet_id='${tweet_id}' AND like_user_id='${user_id}'`
            var delete_result = await queryExecute(like_delete)

            l_count -= 1
            var update_tweet_like = `UPDATE twitter.tweet_master SET like_count=${l_count} 
        ...
        //Re-like updation ------------------------------------------------------------
            ...
            var like_update = `UPDATE twitter.like_master SET activate='1' WHERE like_tweet_id='${tweet_id}' AND like_user_id='${user_id}'`
    ...
    //First time liked so, inserted -----------------------------------------------------
        ...
        var insert_like = `insert into twitter.like_master(like_user_id,like_tweet_id,activate) 
